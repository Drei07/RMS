...
<body>
	...
	<section id="content">
		...
		<!-- MAIN -->
		<main>
			<h1 class="title">G12 Subject Classes</h1>
            <ul class="breadcrumbs">
				<li><a href="home" >Home</a></li>
				<li class="divider">|</li>
				<li><a href="" class="active">List of Classes</a></li>
			</ul>
			<div class="manage">
				<button type="button" class="btn-primary" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#classModal"><i class='bx bxs-plus-circle'></i> Create Class</button>
			</div>
            ...
            <div class="info-data">
				<?php
						...
						$pdoQuery = "SELECT * FROM classes WHERE year_level = :year_level AND teacherId = :teacherId AND status = :status";
						...
						while($row=$pdoResult->fetch(PDO::FETCH_ASSOC)) {
							...
							?>
								<div class="card">
									<div class="head" style="background-color: <?php echo $row['class_color']; ?>;">
									<a href="controller/delete-class-controller?teacherID=<?php echo $uniqueId?>&id=<?php echo $row['Id'] ?>&year_level=<?php echo $row['year_level'] ?>" class="delete"><i class='bx bxs-trash dot' style="color:<?php echo $row['class_color']; ?>;"></i></a>
										<div>											
											<h2><?php echo $row['class_code']; ?></h2>
											
											<?php 

												$subjectId = $row['program'];
												$subject_name = $row['subject_name'];

												$pdoQuery = "SELECT * FROM subjects_$subjectId WHERE subjectId = :subject_name";
												$pdoResult5 = $pdoConnect->prepare($pdoQuery);
												$pdoResult5->execute(array(":subject_name" => $subject_name));

												if($subject_data=$pdoResult5->fetch(PDO::FETCH_ASSOC))
												{
													echo"<p>$subject_data[subject_name]</p>";
												}
												else
												{
													echo"<p></p>";
											...
											<p style="font-weight: 700;"><?php echo $row['semester']; ?></p>
										</div>
									</div>
									<div class="card-body" onclick="location.href='class?id=<?php echo $row['classId'] ?>';">
										<p style="background-color: <?php echo $row['class_color']; ?>;">
										<?php

											$classId = $row['classId'];

											$pdoQuery = "SELECT * FROM class_$classId";
											$pdoResult2 = $pdoConnect->prepare($pdoQuery);
											$pdoResult2->execute();
									
											$count = $pdoResult2->rowCount();
			...
			<div class="modal-dialog modal-dialog-centered modal-lg">
				...
				<div class="header"></div>
					...
					<div class="modal-body">
							<form action="controller/add-class-controller.php" method="POST" class="row gx-5 needs-validation" name="form" onsubmit="return validate()"  novalidate style="overflow: hidden;">
								<div class="row gx-5 needs-validation">
									...
									<div class="col-md-6" style="display: none;">
										...
										<input type="text" class="form-control" autocapitalize="on" maxlength="15" autocomplete="off" name="ClassType" value="<?php echo $class_type  ?>" id="classtype">
									</div>
									...
									<div class="col-md-6">
										...
										<select class="form-select form-control semester"  name="Semester"  autocapitalize="on" maxlength="6" autocomplete="off" id="semester" required>
									...
									<div class="col-md-6">
										...
										<select class="form-select form-control subject"  name="Subject"  autocapitalize="on" maxlength="6" autocomplete="off" id="subject" required>
									...
									<div class="col-md-6">
										...
										<input type="text" onkeyup="this.value = this.value.toUpperCase();" class="form-control" autocapitalize="on" maxlength="15" autocomplete="off" name="ClassCode" id="classCode" required placeholder="eg. HUMSS-101">
										<div class="invalid-feedback">
	...
	<script>
		...
		$(document).ready(function()
		{
			...
			{
				var programs_id = $(this).val();
				$.ajax({
					url:"controller/select-class-controller.php?year_level=<?php echo $year_level ?>",
					method:"POST",
			...
			{
				var programs_id_value = $(".programs").val();
				var semester_id = $(this).val();
				$.ajax({
					url:"controller/select-class-controller.php?year_level=<?php echo $year_level ?>",
					method:"POST",
					data:{programs_id_value:programs_id_value,semester_id:semester_id},
					success:function(data){
						$(".subject").html(data);
					}
				});
