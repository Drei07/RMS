...
else if($advisory_year == "Grade12")
{
	$location = "G12-advisory-class";
}

$pdoQuery = "SELECT * FROM academic_programs WHERE programID = :programID";
$pdoResult2 = $pdoConnect->prepare($pdoQuery);
$pdoResult2->execute(array(":programID" => $advisory_program));
...
<head>
	...
	<title>Class | <?php echo $advisory_section_name ?></title>
</head>
<body>
	...
	<section id="content">
		...
		<!-- MAIN -->
		<main>
			<h1 class="title">SECTION: <?php echo $advisory_section_name ?>	- <label><?php echo $program_name ?></label></h1>
            ...
            <div class="level">
					<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#classModal" style="background-color: <?php echo $advisory_color ?>;"><i class='bx bxs-plus-circle'></i> Add Student</button>
            </div>

            <section class="data-table">

                <div class="searchBx">
                    <input type="input" placeholder="search LRN . . . . . " class="search numbers" inputmode="numeric" name="search_box2" id="search_box2"><button class="searchBtn" style="background-color: <?php echo $advisory_color ?>;"><i class="bx bx-search icon"></i></button>
                </div>
	...
	<script>
        ...
        {
        $.ajax({
            url:"data-table/student2-data-table.php?advisoryId=<?php echo $advisoryId ?>",
            method:"POST",
		...
		{
		$.ajax({
			url:"data-table/advisory-student-data-table.php?advisoryId=<?php echo $advisoryId ?>",
			method:"POST",
			data:{page:page, query:query},
			success:function(data)
			{
